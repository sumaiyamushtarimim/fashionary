...
import { getProductById, getProducts, getCategories } from '@/services/products';
...
export default function EditProductPage() {
  ...
  React.useEffect(() => {
      ...
      Promise.all([
          ...
          getProducts(),
          getCategories()
      ]).then(([productData, productsData, categoriesData]) => {
          ...
          if (productData) {
            form.reset({
              ...
              regularPrice: productData.price || undefined,
              salePrice: undefined,
              ...
              attributes: productData.variants ? [{ name: 'Size', options: 'Small, Medium, Large'}] : [],
              variations: productData.variants?.map(v => ({
                  id: v.id,
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="md:col-span-2 space-y-6">
                        ...
                        <FormField control={form.control} name="slug" render={({ field }) => (
                            <FormItem><FormLabel>Slug</FormLabel><FormControl><Input {...field} /></FormControl><FormDescription>URL-friendly version of the name.</FormDescription><FormMessage /></FormItem>
                        )} />
                        ...
                        {productType === 'variable' && (
                            ...
                            <Card>
                                <CardHeader className="flex flex-row items-center justify-between"><div><CardTitle>Variations</CardTitle><CardDescription>Manage product variations.</CardDescription></div><Button type="button" variant="outline" size="sm" onClick={generateVariations}>Create variations from all attributes</Button></CardHeader>
                                <CardContent>
                                            ...
                                            <AccordionItem value={field.id} key={field.id}>
                                                ...
                                                <AccordionContent>
                                                     <div className="grid grid-cols-2 gap-4">
                                                        <div className="col-span-2 flex items-center gap-4"><div className="border-2 border-dashed border-muted-foreground/50 rounded-md p-4 flex flex-col items-center justify-center text-center w-24 h-24"><UploadCloud className="w-6 h-6 text-muted-foreground" /><span className="text-xs mt-1">Image</span></div>
                                                            <div className="grid grid-cols-2 gap-4 flex-1">
                                                                <FormField control={form.control} name={`variations.${index}.regularPrice`} render={({ field }) => (<FormItem><FormLabel>Regular price</FormLabel><FormControl><Input type="number" placeholder="25.00" {...field} /></FormControl></FormItem>)} />
                                                                 <FormField control={form.control} name={`variations.${index}.salePrice`} render={({ field }) => (<FormItem><FormLabel>Sale price</FormLabel><FormControl><Input type="number" placeholder="19.99" {...field} /></FormControl></FormItem>)} />
                                                            </div>
                                                        </div>
                                                        <FormField control={form.control} name={`variations.${index}.sku`} render={({ field }) => (<FormItem><FormLabel>SKU</FormLabel><FormControl><Input placeholder="TSHIRT-BLK-L" {...field} /></FormControl></FormItem>)} />
                                                     </div>
                                                </AccordionContent>
                                            </AccordionItem>
                                        ))}
                        ...
                        {productType === 'combo' && (
                           <Card>
                                <CardHeader><CardTitle>Combo Products</CardTitle><CardDescription>Select products to include in this combo.</CardDescription></CardHeader>
                                <CardContent>
                                     <FormField control={form.control} name="comboProducts" render={({ field }) => (
                                            ...
                                            <Popover open={popoverOpen} onOpenChange={setPopoverOpen}><PopoverTrigger asChild><FormControl><Button variant="outline" role="combobox" className={cn("w-full justify-between", !field.value?.length && "text-muted-foreground")}>{field.value?.length ? `${field.value.length} selected` : "Select products"}<ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" /></Button></FormControl></PopoverTrigger>
                                                <PopoverContent className="w-[--radix-popover-trigger-width] p-0"><Command><CommandInput placeholder="Search product..." /><CommandList><CommandEmpty>No product found.</CommandEmpty><CommandGroup>
                                                        {allProducts.map((p) => (<CommandItem key={p.id} value={p.name} onSelect={() => { const currentValue = field.value || []; const newValue = currentValue.includes(p.id) ? currentValue.filter((id) => id !== p.id) : [...currentValue, p.id]; field.onChange(newValue);}}>
                                                                <div className={cn("mr-2 flex h-4 w-4 items-center justify-center rounded-sm border border-primary", field.value?.includes(p.id) ? "bg-primary text-primary-foreground" : "opacity-50 [&_svg]:invisible")}><Check className="h-4 w-4" /></div>{p.name}</CommandItem>))}
                                                        </CommandGroup></CommandList></Command></PopoverContent>
                                            </Popover><FormDescription>These products will be bundled together.</FormDescription><FormMessage />
                                            </FormItem>)} />
                                        <div className="mt-4 space-y-2">
                                            <Label>Selected Products</Label>
                                            {comboProductDetails.length > 0 ? (<div className="rounded-md border"><Table><TableBody>
                                                        {comboProductDetails.map(p => (<TableRow key={p.id}><TableCell><Image src={p.image.imageUrl} alt={p.name} width={40} height={40} className="rounded-md" /></TableCell><TableCell className="font-medium">{p.name}</TableCell><TableCell className="text-right">৳{p.price.toFixed(2)}</TableCell></TableRow>))}
                                                        </TableBody></Table></div>) : (<div className="text-sm text-muted-foreground text-center py-4">No products selected.</div>)}
                                        </div>
                                </CardContent>
                           </Card>
                        )}
                        ...
                        <Card><CardHeader><CardTitle>Product Data</CardTitle></CardHeader>
                            <CardContent>
                                <FormField control={form.control} name="productType" render={({ field }) => (
                                        <FormItem><FormLabel>Product Type</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select a product type" /></SelectTrigger></FormControl>
                                                <SelectContent><SelectItem value="simple">Simple product</SelectItem><SelectItem value="variable">Variable product</SelectItem><SelectItem value="combo">Combo product</SelectItem></SelectContent>
                                            </Select><FormMessage /></FormItem>)} />
                            </CardContent>
                        </Card>
                        {(productType === 'simple' || productType === 'combo') && (
                            <Tabs defaultValue="general"><TabsList className="w-full"><TabsTrigger value="general">General</TabsTrigger><TabsTrigger value="inventory">Inventory</TabsTrigger>{(productType === 'simple') && <TabsTrigger value="shipping">Shipping</TabsTrigger>}</TabsList>
                                <TabsContent value="general" className="mt-6">
                                    <Card><CardContent className="pt-6 space-y-4">
                                            <FormField control={form.control} name="regularPrice" render={({ field }) => (<FormItem><FormLabel>Regular price (৳)</FormLabel><FormControl><Input type="number" placeholder="25.00" {...field} disabled={productType === 'combo'} /></FormControl>{productType === 'combo' && <FormDescription>Auto-calculated from selected products.</FormDescription>}<FormMessage /></FormItem>)} />
                                            <FormField control={form.control} name="salePrice" render={({ field }) => (<FormItem><FormLabel>Sale price (৳)</FormLabel><FormControl><Input type="number" placeholder="19.99" {...field} /></FormControl><FormDescription>Leave blank to not have a sale.</FormDescription><FormMessage /></FormItem>)} />
                                        </CardContent></Card>
                                </TabsContent>
                                <TabsContent value="inventory" className="mt-6">
                                    <Card><CardContent className="pt-6 space-y-4">
                                            <FormField control={form.control} name="sku" render={({ field }) => (<FormItem><FormLabel>SKU</FormLabel><FormControl><Input placeholder="COMBO-001" {...field} /></FormControl><FormMessage /></FormItem>)} />
                                              {productType === 'combo' && (<div className="text-sm text-muted-foreground p-4 border rounded-md bg-muted/50"><p className="font-medium">Stock Management</p><p>The stock for a combo product is managed by the inventory of its component products. You do not need to set stock manually.</p></div>)}
                                        </CardContent></Card>
                                </TabsContent>
                                {productType === 'simple' && (<TabsContent value="shipping" className="mt-6">
                                        <Card><CardContent className="pt-6 space-y-4">
                                                <FormField control={form.control} name="weight" render={({ field }) => (<FormItem><FormLabel>Weight (kg)</FormLabel><FormControl><Input type="number" placeholder="0.5" {...field} /></FormControl><FormMessage /></FormItem>)} />
                                                ...
                                                {isThreePieceCategory && (<div className="pt-4"><Label>Fabric Consumption (yards)</Label><div className="grid grid-cols-3 gap-2 mt-2">
                         ...
                         <Card><CardHeader><CardTitle>Organization</CardTitle></CardHeader>
                            <CardContent className="space-y-4">
                               <FormField control={form.control} name="categoryId" render={({ field }) => (
                                ...
                                <FormField control={form.control} name="tags" render={({ field }) => (<FormItem><FormLabel>Tags</FormLabel><FormControl><Input placeholder="Cotton, Eco-friendly" {...field} /></FormControl><FormDescription>Comma-separated values.</FormDescription><FormMessage /></FormItem>)} />
                            </CardContent>
                        </Card>
                    </div>
                  </div>
